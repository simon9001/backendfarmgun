...
export class AdminController {
  ...
  static async getDashboardStats(c: Context) {
    try {
      ...
      const revenue = totalRevenue.data?.reduce((sum: number, payment: { amount: number }) =>
      ...
      const totalBookingsCount = (totalBookings as SupabaseCountResponse).count || 0;
      const completedBookingsCount = (completedBookings as SupabaseCountResponse).count || 0;
      const pendingBookingsCount = (pendingBookings as SupabaseCountResponse).count || 0;
      const userStatsCount = (userStats as SupabaseCountResponse).count || 0;
      const paymentStatsCount = (paymentStats as SupabaseCountResponse).count || 0;
  ...
  private static processTrendData(bookings: BookingTrendData[], period: string): ProcessedTrendData[] {
    const trendMap = new Map<string, { bookings: number; completed: number }>();
  ...
  static async unlinkCropFromService(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async updateCrop(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async deleteCrop(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async createProject(c: Context) {
    try {
      ...
      // Handle extra media if provided
      if (media_ids && media_ids.length > 0) {
        const projectMediaRecords = media_ids.map((media_id: string, index: number) => ({
  ...
  static async addProjectMedia(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async removeProjectMedia(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async approveTestimonial(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async createTip(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async updateTip(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async deleteTip(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  // ==================== BOOKINGS MANAGEMENT ====================
  static async getAllBookings(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async updateBooking(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async cancelBookingAdmin(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async setAvailability(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  static async getAvailability(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
      ...
      for (let hour = startHour; hour < endHour; hour++) {
        for (let minute = 0; minute < 60; minute += slotDuration) {
          ...
          slots.push({
            time,
            available: !isBooked,
            booked_by: isBooked ? bookings?.find(b =>
  ...
  static async updateSystemSettings(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  // ==================== EXPORT DATA ====================
  static async exportData(c: Context) {
    try {
      const user = c.get('user');
      if (user.role !== 'admin') {
        return c.json({ error: 'Admin access required' }, 403);
  ...
  // ==================== HELPERS ====================
  private static async deleteMediaInternal(mediaId: string) {
    try {
  ...
  private static async uploadFileInternal(c: Context, category: MediaCategory) {
    try {
      ...
      const mimeType = CloudinaryService.getMimeTypeFromResourceType(
