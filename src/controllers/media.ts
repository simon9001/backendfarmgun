...
// Define Cloudinary upload options interface
...
// Define optimized image options interface
...
export class MediaController {
  static async uploadMedia(c: Context) {
    try {
  ...
  static async uploadMultipleMedia(c: Context) {
    try {
      const user = c.get('user');
  ...
  static async getMediaById(c: Context) {
    try {
  ...
  static async updateMedia(c: Context) {
    try {
      ...
      if (category !== undefined) updateData.category = category as MediaCategory;
  ...
  static async getOptimizedUrl(c: Context) {
    try {
      const id = c.req.param('id');
      ...
      const optimizationOptions: OptimizedImageOptions = {};
  ...
  static async uploadFromUrl(c: Context) {
    try {
      const user = c.get('user');
      ...
      // Determine media type
      let mediaType: MediaType = 'document';
      if (uploadResult.resource_type === 'image') mediaType = 'image';
      if (uploadResult.resource_type === 'video') mediaType = 'video';
